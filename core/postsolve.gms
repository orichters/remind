...
*** --------A1: for that, calculate the CO2 tax rescale factor---
      ...
      loop(t2$(t2.val eq c_peakBudgYr),
	    pm_taxCO2eq(t,regi)$(t.val gt c_peakBudgYr) = p_taxCO2eq_until2150(t2,regi) + (t.val - t2.val) * cm_taxCO2inc_after_peakBudgYr * sm_DptCO2_2_TDpGtC;  !! increase by cm_taxCO2inc_after_peakBudgYr per year
	  );
