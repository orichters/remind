...
nodamagefolder <- "../remind-3p3p1/output"
nodamagescenarios <- c("d_delfrag", "d_strain", "o_2c", "h_cpol", "o_lowdem_LUtax30", "o_1p5c_LUtax30", "h_ndc")
curpol <- "h_cpol"
...
levels(nodamage$scenario) <- gsub("_LUtax30", "", levels(nodamage$scenario))

runnames <- lucode2::getScenNames(outputdir)

damagestrings <- c(medium = "_KLW_d50_newmagicc")
