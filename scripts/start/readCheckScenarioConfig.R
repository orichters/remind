...
readCheckScenarioConfig <- function(filename, remindPath = ".", testmode = FALSE, fillWithDefault = FALSE) {
  ...
  if (length(unknownColumnNames) > 0) {
    ...
    if (length(unknownColumnNames) > 0) {
      message("\nAutomated checks did not find counterparts ",
              if (isFALSE(coupling)) "in main.gms and default.cfg for these columns in ",
              basename(filename), ":")
      message("  ", paste(unknownColumnNames, collapse = ", "))
      if (coupling %in% "MAgPIE") message("You are not allowed to specify REMIND switches in coupled config.")
