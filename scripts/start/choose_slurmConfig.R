...
choose_slurmConfig <- function(identifier = FALSE, flags = NULL) {
  ...
  if (slurm) { 
    ...
    comp <- switch(as.integer(identifier),
                    ...
                    "4" = "--qos=standby --nodes=1 --tasks-per-node=1 --mem=12000"   , # SLURM standby  - task per node:  1 (nash debug, test one regi)
                    ...
                    "8" = "--qos=priority --nodes=1 --tasks-per-node=1 --mem=12000"  , # SLURM priority - task per node:  1 (nash debug, test one regi)
                   ...
                   "11" = "--qos=short --nodes=1 --tasks-per-node=1 --mem=12000"     , # SLURM short    - task per node:  1 (nash debug, test one regi)
                   "12" = "--qos=medium --nodes=1 --tasks-per-node=1 --mem=12000"    , # SLURM medium   - task per node:  1 (negishi)
                   "13" = "--qos=long --nodes=1 --tasks-per-node=1 --mem=12000"      , # SLURM long     - task per node:  1 (negishi)
